...
/// Result of music selection for Play Songs addon.
...
/// Screen for selecting music playlists or songs for Play Songs addon.
...
class _MusicSelectionScreenState extends State<MusicSelectionScreen>
  ...
  void initState() {
    ...
    _selectedPlaylistName = widget.selectedPlaylistName;
    _selectedTrackIds = Set.from(widget.selectedTrackIds ?? []);
    _songCountLimit = widget.selectedSongCount;
    _initializeProvider();
  }
  ...
  Future<void> _confirmSelection() async {
    if (_selectedPlaylistId != null && _songCountLimit != null && _songCountLimit! > 0) {
      // Load first N tracks from playlist and return as trackIds
      if (_musicProvider != null) {
        try {
          final tracks = await _musicProvider!.getPlaylistTracks(
            _selectedPlaylistId!,
            limit: _songCountLimit!,
          );
          final trackIds = tracks.map((t) => t.id).toList();
          if (mounted) {
            Navigator.of(context).pop(MusicSelectionResult(
              playlistId: _selectedPlaylistId,
              playlistName: _selectedPlaylistName,
              trackIds: trackIds.isNotEmpty ? trackIds : null,
              songCount: _songCountLimit,
            ));
          }
          return;
        } catch (_) {
      ...
      bottomNavigationBar: Container(
        ...
        child: SafeArea(
          child: Column(
            ...
            children: [
              if (_selectedPlaylistId != null && _tabController.index == 0) ...[
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(
                      _songCountLimit == null
                          ? 'Full playlist'
                          : 'Limit to $_songCountLimit songs',
                    ...
                    Row(
                      children: [
                        IconButton(
                          ...
                          onPressed: _songCountLimit == null
                              ? null
                              : _songCountLimit! <= 1
                                  ? () => setState(() => _songCountLimit = null)
                                  : () => setState(() => _songCountLimit = _songCountLimit! - 1),
                        ),
                        Text(
                          _songCountLimit == null ? 'All' : '$_songCountLimit',
                        ...
                        IconButton(
                          ...
                          onPressed: () => setState(() {
                            _songCountLimit = ((_songCountLimit ?? 0) + 1).clamp(1, 50);
                          }),
                        ),
                      ],
                    ),
                  ...
                  Expanded(
                    child: Text(
                      _selectedPlaylistId != null
                          ? (_songCountLimit != null && _songCountLimit! > 0
                              ? '${_selectedPlaylistName ?? 'Playlist'} (${_songCountLimit} songs)'
